...
public class GeoPoint {
	...
	public static double GetDistance(Double latitudeA,Double longitudeA,Double latitudeB,Double longitudeB) {
		double height = Math.abs(latitudeA - latitudeB) * M_PER_LAT; // 110km per  latitude
		double width = Math.abs(longitudeA - longitudeB) * M_PER_LNG; // 70km  per longitude
		return height * height + width * width;
	}
    /**
     * Get the precise distance between to geo points
     ...
     * @throws
     */
    public static double GetPreciseDistance(Double latitudeA,Double longitudeA,Double latitudeB,Double longitudeB)
    {
        double radLatA = rad(latitudeA);
        double radLatB = rad(latitudeB);
        double a = radLatA - radLatB;
        double b = rad(longitudeA) - rad(longitudeB);
        double distance = 2 * Math.asin(Math.sqrt(Math.pow(Math.sin(a / 2), 2) + Math.cos(radLatA) * Math.cos(radLatB) * Math.pow(Math.sin(b / 2), 2)));
        distance = distance * 6378137.0;
        distance = (int)(distance * 10000) / 10000;
        return distance;
    }
